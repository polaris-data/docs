...
When OHLCV appears inside `/events` or snapshot files, it uses the standard event envelope (see [Standardised formats](/reference/standardised-formats)) and the bar-specific fields live under the nested `data` object.
...
- [Standardised formats](/reference/standardised-formats) for the full event envelope OHLCV bars are nested in inside event streams
