...
Use `replay(...)` when you want to stream historical rows in a notebook, script, or backfill job. You can run this without an API key set.
...
| `replay(source, market, from_, to, standard=True)` | Iterator of rows | Stream historical data for replay or backfills |
| `events(source, market, from_, to, limit=1000)` | List of normalized events | Load mixed event types into scripts or notebooks |
| `trades(source, market, from_, to, limit=1000)` | List of normalized trades | Analyze executions |
| `raw(source, market, from_, to, limit=1000)` | List of raw payloads | Inspect venue-native messages |
| `ohlcv(source, market, from_, to, interval, format=None)` | List of bars or TradingView-style JSON | Plot fast market charts |
...
To access the full Polaris dataset (including premium sources, raw snapshots, and historical data beyond the public window), you need a Polaris API key. However, you can get started quickly with free sources and markets like Binance BTC-USDT without any authentication.

**Quickstart without API key:** The examples in the Quickstart, Discover a market, and Query events/trades/OHLCV sections work with public data sources and don't require an API key.

**When you need an API key:** For accessing other datasets, raw snapshots, or making authenticated requests, set your API key either:
1. As an environment variable: `export POLARIS_API_KEY="pk_live_your_key"`
2. Directly in the PolarisClient: `PolarisClient(api_key="pk_live_your_key")`

If you omit `api_key`, the client reads `POLARIS_API_KEY` from the environment. See [Authentication](/guides/authentication) for the shared auth model.
