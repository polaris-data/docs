...
<ParamField query="mode" type="string">
  Optional response mode. Defaults to `redirect`, which issues a `302` to the file. Use `json` to receive a JSON body with a pre-signed URL instead of following the redirect — useful for machine clients that don't handle redirects.
</ParamField>
...
- `mode=json` returns JSON with `url` and `expires_in_seconds` instead of redirecting.
...
<ResponseField name="redirect" type="string">
  The default response mode. The endpoint issues a `302` redirect to the downloadable snapshot file.
</ResponseField>

<ResponseField name="url" type="string">
  Pre-signed download URL when you request `mode=json`.
</ResponseField>
