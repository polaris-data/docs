...
description: "List and download canonical hourly snapshot files for historical ingestion."
api: "GET /snapshots"
authMethod: "none"
---
...
3. Call [Download](/reference/download) with a `key` from the listing to fetch one file.
...
<ResponseField name="snapshots.key" type="string" required>
  Snapshot key to pass to `GET /download`.
</ResponseField>
...
Raw snapshot listing requires bearer auth with a paid subscription. The response uses the same shape but entries contain `date`, `key`, and `filename` without an `hour` field.
  ...
  "snapshots": [
    {
      "date": "2026-05-07",
      "hour": 0,
      "key": "snapshots/hourly/hyperliquid/BTC-USD/2026-05-07/0.jsonl.zst",
...
- Read [Download](/reference/download) to fetch a snapshot file by key.
