...
- Discover which exchanges, assets, and time ranges are available right now.
- Download historical snapshot files for bulk research and ingestion.
- Query recent trades, events, and OHLCV bars over REST.
- Inspect raw exchange-native payloads when replayability matters.
- Reuse the same access model across the CLI, REST API, and SDKs.

## Why teams use Polaris

Most market data work breaks down at the venue layer. Every exchange has its own identifiers, timestamp conventions, payload shapes, and quirks around history. Polaris gives you one place to:
...
<CardGroup cols={2}>
  ...
  <Card title="Browse Datasets" icon="git-branch" href="/guides/choose-the-right-data">
  ...
  <Card title="Catalog" icon="database" href="/reference/catalog">
    Discover supported exchanges, exact asset IDs, and available time bounds.
  </Card>
  ...
  <Card title="API Overview" icon="braces" href="/reference/api-reference">
    Explore the main REST surfaces and jump into endpoint-specific docs.
  </Card>
  ...
  <Card title="Standardised Formats" icon="layers" href="/schemas/standardised-formats">
    Understand the normalized cross-venue event envelope used across Polaris datasets.
  </Card>
</CardGroup>
...
2. Open [Browse Datasets](/guides/choose-the-right-data) if you are choosing between snapshots, REST, and raw payloads.
3. Use [Catalog](/reference/catalog) to confirm exact `exchange` and `asset` pairs.
...
- Use raw payloads when you need exchange-native fields for replay or validation.
- Use the Python SDK when you want a notebook or local analysis workflow.

## Support and resources

- Use [Market Coverage](/markets/market-coverage) for a live high-level view of currently listed exchanges.
- Use [Jupyter Notebook Quickstart](/guides/jupyter-notebook-quickstart) if you want a notebook-first Python workflow.
