...
Use [Catalog](/reference/catalog) to confirm the exact asset ID before you query or download data. Asset IDs are venue-specific. For Hyperliquid, use canonical IDs such as `BTC`.
...
<Steps>
  ...
  <Step title="Browse a market in the terminal">
    ...
    ```bash
    polaris catalog --exchange hyperliquid --asset BTC
    ```
  </Step>
  ...
  <Step title="Inspect local data">
    ...
    ```bash
    polaris list --exchange hyperliquid --asset BTC
    ```
  </Step>
</Steps>
